...
void createLimits(Parameters & parameters, DM & da, int *limitsX, int *limitsY, int *limitsZ){
    ...
    // these are shifted so that the body iterations don't touch the boundaries, which are treated
    // with a differently
    // Check left wall
...
Parameters & PetscUserCtx::getParameters(){
    return _parameters;
}

FlowField & PetscUserCtx::getFlowField(){
    return _flowField;
}
    ...
    for (j = limitsY[0]; j < limitsY[1]; j++){
        for (i = limitsX[0]; i < limitsX[1]; i++){
            ...
            if ((obstacle & OBSTACLE_SELF) == 0) {  // If we have a fluid cell
                ...
                const FLOAT dx_M1= parameters.meshsize->getDx(cellIndexX-1,cellIndexY  );
                const FLOAT dx_P1= parameters.meshsize->getDx(cellIndexX+1,cellIndexY  );
                const FLOAT dy_0 = parameters.meshsize->getDy(cellIndexX  ,cellIndexY  );
                const FLOAT dy_M1= parameters.meshsize->getDy(cellIndexX  ,cellIndexY-1);
                const FLOAT dy_P1= parameters.meshsize->getDy(cellIndexX  ,cellIndexY+1);
    ...
    if (context->setAsBoundary & LEFT_WALL_BIT){
        for (j = limitsY[0]; j < limitsY[1]; j++){
            ...
            if (parameters.walls.typeLeft == DIRICHLET){   // If Dirichlet velocity boundary conditions
                ...
                stencilValues[0] = 1; stencilValues[1] = -1;
...
PetscErrorCode computeMatrix3D(KSP ksp, Mat A, Mat pc, void* ctx){
#else
PetscErrorCode computeMatrix3D(KSP ksp, Mat A, Mat pc, MatStructure * matStructure, void* ctx){
#endif
    PetscUserCtx* context = (PetscUserCtx*) ctx;
    Parameters & parameters = context -> getParameters();
        ...
        for (j = limitsY[0]; j < limitsY[1]; j++){
            for (i = limitsX[0]; i < limitsX[1]; i++){
                ...
                if ((obstacle & OBSTACLE_SELF) == 0) { // If the cell is fluid
                    ...
                    const FLOAT dy_0 = parameters.meshsize->getDy(cellIndexX  ,cellIndexY  ,cellIndexZ  );
                    const FLOAT dy_M1= parameters.meshsize->getDy(cellIndexX  ,cellIndexY-1,cellIndexZ  );
                    const FLOAT dy_P1= parameters.meshsize->getDy(cellIndexX  ,cellIndexY+1,cellIndexZ  );
                    const FLOAT dz_0 = parameters.meshsize->getDz(cellIndexX  ,cellIndexY  ,cellIndexZ  );
                    const FLOAT dz_M1= parameters.meshsize->getDz(cellIndexX  ,cellIndexY  ,cellIndexZ-1);
                    const FLOAT dz_P1= parameters.meshsize->getDz(cellIndexX  ,cellIndexY  ,cellIndexZ+1);
			...
			counter_fluid++;
                ...
                column[0].i = i+1; column[0].j = j; column[0].k = k;
                column[1].i = i-1; column[1].j = j; column[1].k = k;
                column[2].i = i;   column[2].j = j; column[2].k = k;
                column[3].i = i;   column[3].j = j+1; column[3].k = k;
                column[4].i = i;   column[4].j = j-1; column[4].k = k;
                column[5].i = i;   column[5].j = j; column[5].k = k+1;
                column[6].i = i;   column[6].j = j; column[6].k = k-1;
                MatSetValuesStencil(A, 1, &row, 7, column, stencilValues, INSERT_VALUES);
            }
            }
        }
...
const DM & PetscSolver::getGrid() const{
    return _da;
}
...
void PetscUserCtx::setRank(int rank){
    _rank = rank;
}

int PetscUserCtx::getRank() const {
    return _rank;
}
